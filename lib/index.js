...
class GradientColor {
    ...
    setMidpoint(minNumber, maxNumber) {
    ...
    getColor(numberValue) {
        ...
        return ('#' +
            this.generateHex(numberValue, this.startColor.substring(0, 2), this.endColor.substring(0, 2)) +
            this.generateHex(numberValue, this.startColor.substring(2, 4), this.endColor.substring(2, 4)) +
            this.generateHex(numberValue, this.startColor.substring(4, 6), this.endColor.substring(4, 6)));
    }
    generateHex(numberValue, start, end) {
...
class Gradient {
    ...
    getColor(numberValue) {
        if (isNaN(numberValue)) {
            ...
            const index = Math.min(Math.floor((Math.max(numberValue, 0) - 0) / segment), this.gradients.length - 1);
            return this.gradients[index].getColor(numberValue);
        }
    }
    setMidpoint(maxNumber) {
